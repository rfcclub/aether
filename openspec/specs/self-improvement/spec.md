...
## Purpose
The self-improvement system runs a daily review cron, detects friction signals in session history, promotes high-confidence observations to durable memory, generates skill patches from recurring failures, gates patches behind benchmark verification, and tracks all candidates through a visible state pipeline.
