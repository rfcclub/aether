...
public sealed class WebSocketChannelService : BackgroundService
{
    ...
    private async Task HandleMessageAsync(InboundMessage message, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Access control — gate before routing (always allowed for localhost websocket)
            var access = _channel.Name.Equals("websocket", StringComparison.OrdinalIgnoreCase)
                ? AccessResult.Allowed
                : await _channelAccess.CheckAccessAsync(message.SenderId, linkedToken);
            switch (access)
            {
